...
def format_material_result_line(number: int, name: str, qty: float, price: float, cost: float) -> str:
    ...
    return f"{number}. {name}: {qty_str} шт × {price_str} = {cost_str}"
...
def format_explanation() -> str:
    """
    Возвращает текст пояснения по расчётам с правильным Markdown
    """
    return """📖 ПОЯСНЕНИЕ ПО ЦИФРАМ

💰 МАТЕРИАЛЫ
• Сумма всех затрат на материалы
• Рассчитывается как: количество × цена за 1 шт
...
• Стоимость чертежа узла = цена производства узла × (нужное количество / кратность узла)
...
• Формула: расход = базовый_расход × (эффективность / 150)
